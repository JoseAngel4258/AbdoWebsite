import React from 'react'

const Lupo = () => {
  return (
    <section className="w-screen flex flex-col gap-20">
      <div className="h-20"></div>
      <div className="flex flex-col items-center justify-center">
        <h1 className="text-[40px]">Cambio de contraseña Router TP-LINK </h1>

        <iframe
          className="my-10"
          width="640"
          height="340"
          src="https://www.youtube.com/embed/dQw4w9WgXcQ"
          title="YouTube video player"
          frameborder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          allowfullscreen
        ></iframe>
        <p className="text-lg">
          Para administrar el router deben ingresar a la dirección: 192.168.1.1{" "}
          <br />
          <br />
          Usuario: admin <br />
          <br /> Contraseña: stdONU101. <br />
          <br />
          El nombre del wifi lo cambian en el mismo menú, la opción se llama
          "SSID".
        </p>
      </div>

      <div className="flex flex-col items-center justify-center">
        <h1 className="text-[40px]">Tutorial de lista blanca TP-LINK    </h1>

        <iframe
          className="my-10"
          width="640"
          height="340"
          src="https://www.youtube.com/embed/dQw4w9WgXcQ"
          title="YouTube video player"
          frameborder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          allowfullscreen
        ></iframe>
        <p className="text-lg">
          Para administrar el router deben ingresar a la dirección: 192.168.1.1{" "}
          <br />
          <br />
          Usuario: admin <br />
          <br /> Contraseña: stdONU101. <br />
          <br />
          El nombre del wifi lo cambian en el mismo menú, la opción se llama
          "SSID".
        </p>
      </div>

      <div className="flex flex-col items-center justify-center">
        <h1 className="text-[40px]">Tutorial de lista negra TP-LINK </h1>

        <iframe
          className="my-10"
          width="640"
          height="340"
          src="https://www.youtube.com/embed/dQw4w9WgXcQ"
          title="YouTube video player"
          frameborder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
          allowfullscreen
        ></iframe>
        <p className="text-lg">
          Para administrar el router deben ingresar a la dirección: 192.168.1.1{" "}
          <br />
          <br />
          Usuario: admin <br />
          <br /> Contraseña: stdONU101. <br />
          <br />
          El nombre del wifi lo cambian en el mismo menú, la opción se llama
          "SSID".
        </p>
      </div>
      <div className="h-20"></div>
    </section>
  );
}

export default Lupo